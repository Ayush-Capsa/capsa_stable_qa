...
class _WithdrawPageState extends State<WithdrawPage> {
  ...
  void verifyData() async {
    ...
    var response = await callApi('/dashboard/r/checkRestriction', body: _body);
    capsaPrint('Response restriction : $response');

    if (response['msg'] == null || response['msg'] != 'success') {
      showToast(response['msg']??'Internal Error!', context, type: 'warning');
      ...
      if (response['isPresent'] == 'false') {
        ...
        Beamer.of(context).beamToReplacementNamed('/change-transaction-pin');
      } else {
        //Navigator.of(context).pop(true);
        setState(() {
          dataVerified = true;
  ...
  void initiateTransaction(ProfileProvider _profileProvider) async {
    ...
    if (response['res'] == 'failed') {
      if (response['messg'] ==
        ...
        Beamer.of(context)
            .beamToReplacementNamed('/withdraw-response', data: response);
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    accountNumberController = TextEditingController(
        text: profileProvider.getBankDetails.bene_account_no);
                                          ...
                                          if (v.length > 4) {
                                            ...
                                            pinController.selection =
                                                TextSelection.fromPosition(
                                                    TextPosition(
                                                        offset: pinController
                                                            .text.length));

                                            showToast(
                                                'Pin Cannot be greater than 4 digits',
                                                context,
                                                type: 'warning');
                                          }
                                          _reEnterPin = v;
