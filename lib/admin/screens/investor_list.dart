...
class InvestorList extends StatefulWidget {
  ...
  const InvestorList({Key key, this.title}) : super(key: key);
                                  ...
                                  children: <Widget>[
                                    TextFormField(
                                      ...
                                      decoration: InputDecoration(
                                        hintText:
                                            'Search by name, bvn, rc number & mobile',
                                        // suffixIcon: Icon(
                                        //   Icons.search,
      ...
      body: MediaQuery.of(context).size.width < 800
          ? FutureBuilder<Object>(
              future: Provider.of<ActionModel>(context, listen: false)
                  .queryInvestorList(filter: widget.filter),
              builder: (BuildContext context, AsyncSnapshot<Object> snapshot) {
                if (snapshot.hasData) {
                  ...
                  List<InvestorData> _enquiryData = <InvestorData>[];

                  if (_data['res'] == 'success') {
                    var _results = _data['data']['results'];

                    _results.forEach((element) {
                      _enquiryData.add(InvestorData(
                        element['req_name'],
                        element['NAME'],
                        element['PAN_NO'],
                        element['email'],
                  ...
                  final InvestorListDataSource _investorListDataSource =
                      InvestorListDataSource(
                          context, _enquiryData, widget.title);

                  return BuyerListCard(
